...
with tab_modifica:
    ...
    try:
        ...
        query_ordini = """
            SELECT t.id, t.numero_ordine, t.cliente_id, c.nome as cliente_nome, 
                   ...
                   t.data_creazione,
                   COALESCE(t.origine, 'manuale') as origine,
                   COALESCE(t.da_verificare, false) as da_verificare
            FROM ordini_testata t
            JOIN clienti c ON t.cliente_id = c.id
            WHERE t.stato = 'Nuovo'
            ORDER BY t.da_verificare DESC, t.numero_ordine DESC;
        """
    ...
    if df_ordini_nuovi.empty:
        ...
        n_da_verificare = int(df_ordini_nuovi['da_verificare'].sum())
        if n_da_verificare > 0:
            st.warning(f"⚠️ {n_da_verificare} ordine/i creato/i automaticamente da email necessitano di verifica.")

        opzioni_ordini = {"-- Seleziona un ordine da modificare --": None}
        for _, row in df_ordini_nuovi.iterrows():
            badge = "⚠️ DA VERIFICARE — " if row['da_verificare'] else ("📧 " if row['origine'] == 'email' else "")
            opzioni_ordini[f"{badge}Ordine N° {row['numero_ordine']} - Per: [{row['codice_cliente']} {row['codice_destinazione']}] {row['cliente_nome']}"] = row['id']
        ...
        if id_ordine_target:
            ...
            with st.form("form_modifica_ordine"):
                ...
                if btn_salva_modifiche:
                    try:
                        with conn.session as session:
                            # L'ordine è stato revisionato da un operatore: rimuove l'eventuale flag di verifica
                            session.execute(
                                text("UPDATE ordini_testata SET da_verificare = false WHERE id = :id;"),
                                params={"id": id_ordine_target}
                            )
            ...
            with st.expander("🚨 Zona Pericolo — Elimina Interamente l'Ordine"):
                ...
                if st.button("🗑️ Elimina Ordine e Fallo Sparire", type="primary", disabled=not conferma_cancellazione, key=f"btn_del_{id_ordine_target}"):
                    try:
                        ...
                        st.error(f"Errore durante l'eliminazione dell'ordine: {e}")import streamlit as st
import pandas as pd
from sqlalchemy import text  # Importato text per compatibilità SQLAlchemy 2.0
