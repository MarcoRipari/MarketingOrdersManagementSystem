...
if df_pronti.empty:
    ...
    if id_ordine_target:
        ...
        with col_destra:
            ...
            # --- TRASFORMATO DA st.form A st.container PER ABILITARE I CALLBACK IN TEMPO REALE ---
            ...
            with st.container(border=True):
                ...
                with col_colli:
                    # Funzione trigger immediata al click di + o -
                ...
                # --- BLOCCO ALERT DI CONFERMA (Intercettazione + o -) ---
                    ...
                    if proposta_modifica != st.session_state[key_colli_confermati]:
                        ...
                        with c_no:
                            if st.button("❌ Annulla", key=f"btn_no_{id_ordine_target}", use_container_width=True):
                                st.session_state[key_alert_attivo] = False
                                # Forziamo il widget a tornare al valore precedentemente salvato
                                st.session_state[key_widget_colli] = st.session_state[key_colli_confermati]
                                st.rerun()

                st.write("")
                # Cambiato da st.form_submit_button a st.button standard
                ...
                if btn_spedisci:
                    ...
                    if corriere_sel == "-- Seleziona un Corriere --" or (corriere_sel == "Altro" and not corriere_specifico.strip()):
                        ...
                        try:
                            ...
                            # Pulizia stato dell'ordine elaborato
                            ...
                            st.error(f"Errore transazionale durante la chiusura della spedizione: {e}")import streamlit as st
import pandas as pd
from sqlalchemy import text
